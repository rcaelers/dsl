...
impl GraphNodeSemantics for ParallelDecoderBuilder {
    ...
    fn input_port(
        ...
        match socket.definition_index() {
            0 => Some("strobe".into()),
            1 => Some(format!("d{}", socket.member_index())),
            2 => Some("cs".into()),
...
impl GraphNodePresentation for ParallelDecoderBuilder {
    fn decoder_table_column(
        ...
        super::presentation::parallel_table_column(socket.definition_index())
    }
    ...
    fn word_display_format(&self, _socket: SocketReference<'_>, state: &Value) -> Option<String> {
        Self::parsed(state)
            .ok()
