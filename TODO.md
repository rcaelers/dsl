...
- [tests.architecture-structural] (P2 · medium) Replace the source-text architecture tests with structural
  checks. About 720 lines remain across the workspace that `include_str!` a sibling file and assert on
  ...
  have been removed; graph nodes, compiler, and viewer retain only three explicitly documented checks
  for constraints that compiled structure cannot detect. The viewer's complete workspace dependency
  surface is now asserted structurally. Continue with the signal-tier and UI service source-text suites.
