...
- [tests.architecture-structural] (P2 · medium) Replace the source-text architecture tests with structural
  checks. About 593 lines remain across the workspace that `include_str!` a sibling file and assert on
  ...
  signal capture, UI live capture, and the UI host service retain only eight explicitly documented
  checks for constraints that compiled structure cannot detect. The viewer, derived-data,
  signal-runtime, and signal-capture workspace dependency surfaces are now asserted structurally, as
  is the UI prohibition on host-adapter, concrete-node, and shared-test composition dependencies. UI
  ...
  Continue with the capture-session source-text suite.
