...
`logic_analyzer_graph_capabilities` owns plugin capability contracts through its `node` and `node_support`
namespaces. `logic_analyzer_graph_registry` owns graph-node and payload registration descriptors,
...
`logic_analyzer_graph_registry`, which depends on `logic_analyzer_graph_capabilities`. The compiler crate
never depends on the built-in-node crate. A plugin depends on the capabilities and registry crates plus the
lower-level domains required by its own implementation; it does not depend on the compiler or
application UI.

### Graph Capabilities contract

`logic_analyzer_graph_capabilities` is the supported capability contract. It supplies
directory-backed `node` and `node_support` namespaces; compiler, built-in-node, plug-in, and UI
composition depend on that contract without making the capabilities crate depend back on any of them. Its detailed
ownership and supported surface are defined by the
[`logic_analyzer_graph_capabilities` crate design](../crates/logic_analyzer_graph_capabilities.md).

### Graph compiler and runtime facades

`logic_analyzer_graph_compiler` owns validation, discovery, document edits, and deterministic
lowering. `logic_analyzer_graph_plan` owns the neutral `ProcessingGraph` contract.
`logic_analyzer_graph_runtime` owns materialization, source preparation, cache policy, and run lifecycle.
The compiler imports graph-node capabilities from `logic_analyzer_graph_capabilities` and catalog access
from `logic_analyzer_graph_registry`, then resolves those capabilities into the processing graph.
The compiler and runtime import the graph-plan contract but do not import each other. The runtime
also imports neither the registry nor editable graph documents.

`GraphLowerer` owns an immutable `GraphRegistry` snapshot and provides these application-facing
operations:

- lower and validate a graph;
- discover capture and trigger features;
- apply a feature edit through the owning node;
- resolve sampling overlays and produce `ProcessingGraph`.

`GraphRuntime` receives only processing graphs and injected execution services. It prepares finite
sources, maintains caches, materializes processing plans, and reconciles active runs.

`logic_analyzer_graph_orchestration` owns the worker messages, codecs, client, and worker-side
composition that necessarily invokes a compiler before passing its processing graph to a runtime.
...
results. The production adapter owns separate `GraphLowerer` and `GraphRuntime` values: Run and
live-apply operations lower the current document first and pass the result to the runtime. UI tests
...
registry snapshot and does not expose an editor-registry operation.
...
the runtime assigns a protocol-neutral persistent cache key to every sampling overlay. The store
...
The execution contract belongs to the graph-plan crate-root facade: `ProcessingGraph`,
`ProcessingNode`, `ProcessingEdge`, `ProcessingGraphError`, subscriptions, and resolved sampling
candidates. Runtime result types include `ApplyError`, `LiveRun`, run data, source-preparation
state, and cache results. The compiler owns document discovery wrappers; orchestration owns worker
messages.
...
`logic_analyzer_graph_registry`; renderer factories are submitted through the generic viewer registry
...
The graph-node and payload inventory collection types are declared by
`logic_analyzer_graph_registry`. Built-in nodes and plugins submit registrations there. Compiler,
runtime, and UI consumers read the same validated snapshot or registration sequence without
importing any submitter.

Every enabled submission crate exposes an idempotent `link()` anchor. Native and web application
composition references those anchors before constructing `GraphLowerer` and `GraphRuntime`. The anchor exists only
...
`RunData` is the runtime-owned application-facing snapshot for both initial materialization and
...
renderer keys, and groups and orders the lanes. Consequently neither the graph capabilities nor the
compiler owns panel-facing lane IDs, track IDs, renderer objects, or registry mutation.

`logic_analyzer_graph_capabilities` has no dependency on `egui` or `logic_analyzer_viewer`. Concrete node
...
- graph capabilities does not depend on the compiler, built-in nodes, processing implementations, UI, or
  capture export;
- graph registry does not depend on compiler, runtime, built-in nodes, UI, platform, or target
  APIs;
...
Lowering produces a `ProcessingGraph` containing canonical node state, resolved input kinds,
resolved materializer handles, payload behavior, output subscriptions, stable runtime names, and
edges with concrete port names and buffer sizes. It prunes to sink-reachable
...
publishes its cache and index as they become available. The graph runtime owns preparation
orchestration while
...
Indexed preparation is submitted through a graph-runtime-owned task-executor contract. The native
...
logic_analyzer_ui ──> logic_analyzer_graph_compiler, logic_analyzer_graph_runtime,
                      logic_analyzer_graph_orchestration, logic_analyzer_graph_plan,
                      logic_analyzer_graph_nodes, logic_analyzer_viewer
```
...
compiles documents, starts or updates runtime runs with processing graphs, and consumes run data
through explicit UI service operations.
