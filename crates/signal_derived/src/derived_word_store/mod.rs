//! # `signal_derived::derived_word_store` Design
//!
...
//!
//! - `crates/signal_derived/src/derived_word_store/`;
//! - `crates/signal_derived/src/derived_data_collector/`;
...
//! - [`signal_derived` Design](../../../docs/crates/signal_derived.md);
...
//!
//! ```rust
//! use signal_derived::derived_word_store::{
...
//!
//! The store owns encoded-lane persistence and query behavior, not protocol
//! decoding, viewer rendering, application cache policy, or host filesystem access.
//!
...
//!
//! A **complete block** is encoded and published before readers can observe it. A
//! live writer additionally publishes an immutable **hot tail**, but never exposes a
//! later range ahead of an uncommitted prefix. A **generation** changes when visible
//! data changes, allowing presentation code to invalidate a bounded snapshot safely.
//!
//! [`AnnotationQuery`] is the storage-format-independent read surface: use presence
...
//! cache; the injected [`platform_artifacts::ArtifactRepository`] selects native, browser, or
//! memory storage without changing the API or lane behavior.
//!
//! # Related APIs
//!
//! [`crate::DerivedDataCollector`] materializes retained outputs through registered
//! payload adapters. The viewer consumes the resulting opaque query handles through
//! `CollectedLaneQuery`; graph cache selection and UI presentation are separate
//! consumers of this generic contract.
