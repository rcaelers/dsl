...
/// Host-facing description of a selectable waveform output.
#[derive(Clone, Debug, PartialEq, Eq)]
pub struct ViewerOutputSelection {
    pub node: NodeId,
    pub output: usize,
    pub output_id: String,
    pub label: String,
    pub selected: bool,
    pub indicator_outputs: Vec<usize>,
}
...
pub(crate) fn synchronize_viewer_selections(
    ...
    let saved = match graph.extension::<SavedSelections>(EXTENSION) {
        Ok(saved) => saved,
        Err(error) => {
            return Ok(vec![GraphCompatibilityWarning {
                node: None,
                message: format!(
    ...
    let selections = viewer_output_selections(graph, registry);
    ...
    Ok(had_legacy
        .then(|| GraphCompatibilityWarning {
            node: None,
