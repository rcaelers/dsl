...
- [tests.architecture-structural] (P2 · medium) Replace the source-text architecture tests with structural
  checks. About 760 lines remain across the workspace that `include_str!` a sibling file and assert on
  ...
  have been removed; graph nodes and compiler retain only the two explicitly documented checks for
  constraints that compiled structure cannot detect. Continue with the viewer, signal-tier, and UI
