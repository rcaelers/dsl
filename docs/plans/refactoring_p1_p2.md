...
**Problem.** ~720 lines of `architecture_tests.rs` remain across the workspace that `include_str!`
sibling files and assert `.contains("…")` (largest: `signal_derived` 68 lines,
`logic_analyzer_viewer` 66, and `signal_runtime` 60). They break on renames, pass when the string
...
2. Prioritize the signal-tier and UI service suites. Do not replace an implementation-text
