...
Logic: Packet Framer, Word Field Extractor, Word Matcher (masked comparison, inclusive-range and
set predicates, every-Nth selection, holdoff, optional explicit rearm, and matching-word output),
Edge Detector (edge selection, debounce, and preceding-pulse qualification), Event Gate (signal-controlled trigger filtering),
Event Control (delay, holdoff, and optional explicit rearm), SR Flip-Flop, Logic Gate (op enum
retitles the node; NOT caps the variadic group at one), Buffer, Counter, String Formatter. Sinks: File Writer (inline save
