...
| `node_graph` | `api` | `api` is the sole import path for node-definition contracts and compatibility re-exports of document records used by graph-node implementations. The crate root exposes only the widget/editor composition surface and its typed graph-snapshot serialization failure used by UI hosts. |
...
generic capture-source feature contract. Saved-state failures retain `PersistedStateError` and its
JSON codec cause rather than converting it to display text. `logic_analyzer_graph_plan` owns the typed discovery result
