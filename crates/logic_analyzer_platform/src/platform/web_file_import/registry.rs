...
use signal_processing::{ChunkedByteSource, PreparedByteSource, SourceIdentity};
...
pub(crate) struct ImportedFile {
    ...
    pub(crate) source: Arc<dyn PreparedByteSource>,
}
...
impl BrowserFileRegistry {
    ...
    pub(crate) fn register_chunks(
        ...
        if let Some((reference, _)) = state.files.iter().find(|(_, imported)| {
            imported.display_name == display_name && imported.source.identity() == identity
        }) {
