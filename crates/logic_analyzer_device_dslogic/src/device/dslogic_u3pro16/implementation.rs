...
//! The host supplies USB transport through the U3Pro16 transport contract, so
//! packet construction and capture behavior remain portable.
...
pub(crate) struct DsLogicU3Pro16<T: UsbTransport> {
    transport: T,
...
mod tests {
    ...
    fn packet_fixture() -> PacketFixture {
        serde_json::from_str(include_str!("../../../test_data/buffered_packet.json")).unwrap()
    }

    fn advanced_trigger_packet_fixture() -> AdvancedTriggerPacketFixture {
        serde_json::from_str(include_str!(
            "../../../test_data/advanced_trigger_packet.json"
        ))
        .unwrap()
    }
    ...
    fn streaming_provider_publishes_overflow_as_an_integrity_failure() {
        ...
        assert!(matches!(
            error,
            signal_capture_session::AcquisitionError::Integrity(_)
        ));
