...
The crate depends on `signal_artifacts`, `signal_capture`, `signal_runtime`, and portable support
libraries. It has no dependency on acquisition sessions, graph crates,
...
Shared persistence time and integrity primitives come from `signal_artifacts`, so derived and
